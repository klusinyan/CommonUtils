...
//  Modified by Karen Lusinyan on  02/07/2014.
...
#define kTabOriginX 0.0
#define kTabOriginY 0.0
...
// Options
@property (nonatomic) NSNumber *tabOriginX;
@property (nonatomic) NSNumber *tabOriginY;
...
@synthesize tabOriginX = _tabOriginX;
@synthesize tabOriginY = _tabOriginY;
...
- (NSNumber *)tabOriginX {
    ...
    if (!_tabOriginX) {
        CGFloat value = kTabOriginX;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabOriginX withDefault:value];
        self.tabOriginX = [NSNumber numberWithFloat:value];
    }
    return _tabOriginX;
}
- (NSNumber *)tabOriginY {
    ...
    if (!_tabOriginY) {
        CGFloat value = kTabOriginY;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabOriginY withDefault:value];
        self.tabOriginY = [NSNumber numberWithFloat:value];
    }
    return _tabOriginY;
}
- (NSNumber *)tabHeight {
...
- (void)setNeedsReloadTabViews
{
    for (int i = 0; i < self.tabCount; i++) {
        if ([[self tabViewAtIndex:i] subviews] > 0) {
            UIView *contentView = [[self tabViewAtIndex:i] contentView];
            if (self.dataSource && [self.dataSource respondsToSelector:@selector(viewPager:contentViewForTab:atIndex:)]) {
                [self.dataSource viewPager:self contentViewForTab:contentView atIndex:i];
            }
        }
    }
}

- (void)setNeedsReloadControllers
{
    for (int i = 0; i < self.tabCount; i++) {
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(viewPager:controllerForTab:atIndex:)]) {
            [self.dataSource viewPager:self controllerForTab:[self viewControllerAtIndex:i] atIndex:i];
        }
    }   
}
...
- (void)defaultSettings {
    ...
    // Check absolutly subviews...
    if ([self.pageViewController.view.subviews count] > 0 &&
        [[self.pageViewController.view.subviews objectAtIndex:0] isKindOfClass:[UIScrollView class]]) {
        ...
        DebugLog(@"Dude, can not setup viewPager, please see: %@", NSStringFromSelector(_cmd));
    }
}
- (void)defaultSetup {
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake([self.tabOriginX floatValue],
                                                                     [self.tabOriginY floatValue],
                                                                     [self.tabWidth floatValue]+[self.tabOriginX floatValue],
                                                                     [self.tabHeight floatValue]+[self.tabOriginY floatValue])];
        [tabView addSubview:tabViewContent];
        [tabView setClipsToBounds:YES];
        [tabView setIndicatorColor:self.indicatorColor];
        tabViewContent.center = tabView.center;

        tabView.contentView = tabViewContent;
